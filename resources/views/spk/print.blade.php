...
<head>
    ...
    <title>{{ $spk->spk_number }} - Print</title>
    <style>
        @page {
...
<body>
    <div class="no-print">
        <button onclick="window.print()">🖨️ Print SPK</button>
    ...
    <div class="kop-surat">
        ...
        <div class="spk-number">
            <strong>{{ $spk->spk_number }}</strong><br>
    ...
    <div class="info-container">
        <div class="info-box">
            <div class="info-box-title">Produksi</div>
            <table class="info-table">
                <tr>
                    <td class="label">Tgl Prod.</td>
                    <td class="colon">:</td>
                    <td>{{ $spk->production_date?->format('d/m/Y') ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Target</td>
                    <td class="colon">:</td>
                    <td>{{ $spk->deadline?->format('d/m/Y') ?? '-' }}</td>
                </tr>
            </table>
        </div>
        <div class="info-box">
        ...
        <table class="main-table">
            <thead>
                <tr>
                    ...
                    <th>Nama Produk</th>
                    <th style="width: 60px;" class="center">Target</th>
                    <th style="width: 50px;" class="center">Satuan</th>
                    <th style="width: 60px;" class="center">Aktual</th>
                </tr>
            </thead>
            <tbody>
                @php $outNo = 1; @endphp
                @foreach($spk->items->where('item_type', 'output') as $item)
                    <tr>
                        <td class="center">{{ $outNo++ }}</td>
                        <td>{{ $item->product->code }}</td>
                        <td>{{ $item->product->name }}</td>
                        <td class="right">{{ number_format((float) $item->quantity_planned, 2) }}</td>
    ...
    <div class="signatures-row">
        <div class="signature-box">
            ...
            <div class="space"></div>
            <div class="line">{{ $spk->creator->name }}</div>
        </div>
        <div class="signature-box">
            <div class="title">Disetujui Oleh</div>
            <div class="space"></div>
            <div class="line">{{ $spk->approver?->name ?? '........................' }}</div>
        </div>
        <div class="signature-box">
