<x-app-layout>
    @section('title', 'Tambah Material/Packaging')

    <x-slot name="header">
        <h2 class="text-2xl font-bold text-zinc-900 dark:text-white">Tambah Material/Packaging</h2>
        <p class="text-zinc-500 dark:text-zinc-400 mt-1">Form penambahan material/packaging baru</p>
    </x-slot>

    <div class="max-w-2xl">
        <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6">
            <form method="POST" action="{{ route('inventory.packaging.store') }}" class="space-y-5">
                @csrf
