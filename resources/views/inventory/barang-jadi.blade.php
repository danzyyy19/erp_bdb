    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div class="flex gap-2">
            @if(auth()->user()->isOwner() || auth()->user()->isOperasional() || auth()->user()->isFinance())
                <a href="{{ route('inventory.barang-jadi.create') }}"
                    class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Tambah Barang Jadi</a>
            @endif
            <a href="{{ route('inventory.barang-jadi.history') }}"
        ...
        <x-export-buttons excel-route="export.products.excel" pdf-route="export.products.pdf"
            :excel-params="['category_type' => 'barang_jadi']" :pdf-params="['category_type' => 'barang_jadi']" />
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody id="table-body">
                    @include('inventory.partials.barang-jadi-table')
                </tbody>
            </table>
        </div>
            ...
            function inventoryFilter() {
                return {
                    ...
                    fetchData() {
                        ...
                        const newUrl = '{{ route('inventory.barang-jadi') }}' + (params.toString() ? '?' + params.toString() : '');
